...
seurat_object2[["percent.mt"]] <- PercentageFeatureSet(seurat_object2, pattern = "^MT-")
seurat_object3[["percent.mt"]] <- PercentageFeatureSet(seurat_object3, pattern = "^MT-")

# Filter cells that have unique feature counts over 2,500 or less than 200
seurat_object1 <- subset(seurat_object1, subset = nFeature_RNA > 200 & nFeature_RNA < 2500)
seurat_object2 <- subset(seurat_object2, subset = nFeature_RNA > 200 & nFeature_RNA < 2500)
seurat_object3 <- subset(seurat_object3, subset = nFeature_RNA > 200 & nFeature_RNA < 2500)

# Filter cells that have >5% mitochondrial counts
seurat_object1 <- subset(seurat_object1, subset = percent.mt < 5)
seurat_object2 <- subset(seurat_object2, subset = percent.mt < 5)
seurat_object3 <- subset(seurat_object3, subset = percent.mt < 5)

# Apply LogNormalize which includes a log transformation for each object
...
seurat_object2 <- FindVariableFeatures(seurat_object2, selection.method = "vst", nfeatures = 2000)
seurat_object3 <- FindVariableFeatures(seurat_object3, selection.method = "vst", nfeatures = 2000)

# Find integration anchors
anchors <- FindIntegrationAnchors(object.list = list(seurat_object1, seurat_object2, seurat_object3), dims = 1:20)

# Integrate data based on the anchors
seurat_object <- IntegrateData(anchorset = anchors, dims = 1:10)

# Save the Seurat object
saveRDS(seurat_object, file = "C:/Users/Administrator/Desktop/seurat analysis/integrated_OO18_OO20_OO33_samples/seurat_object_integrated_OO18_OO20_OO33_samples.rds")

# Load the saved Seurat object
seurat_object <- readRDS(file = "C:/Users/Administrator/Desktop/seurat analysis/integrated_OO18_OO20_OO33_samples/seurat_object_integrated_OO18_OO20_OO33_samples.rds")
...
# Scale the data, regressing out unwanted sources of variation (e.g., percent mitochondrial content)
...
png("C:/Users/Administrator/Desktop/seurat analysis/integrated_OO18_OO20_OO33_samples/plot_integration_variable_feature_2000.png", width = 1000, height = 800)
plot1 + plot2
dev.off()


## Perform linear dimensional reduction PCA on the integrated and scaled data
...
png("C:/Users/Administrator/Desktop/seurat analysis/integrated_OO18_OO20_OO33_samples/heatmap_integration_pca_1_to_6.png", width = 1600, height = 1600, res=150)
DimHeatmap(seurat_object, dims = 1:6, cells = 500, balanced = TRUE)
dev.off() 


# Cluster the cells, Find neighbors and clusters
seurat_object <- FindNeighbors(seurat_object, dims = 1:10)
seurat_object <- FindClusters(seurat_object, resolution = 0.5)
seurat_object <- FindClusters(seurat_object, resolution = 0.8)
seurat_object <- FindClusters(seurat_object, resolution = 1.5)
...
# Save the Seurat object
saveRDS(seurat_object, file = "C:/Users/Administrator/Desktop/seurat analysis/integrated_OO18_OO20_OO33_samples/seurat_object_integrated_OO18_OO20_OO33_02_samples.rds")

# Load the saved Seurat object
seurat_object <- readRDS(file = "C:/Users/Administrator/Desktop/seurat analysis/integrated_OO18_OO20_OO33_samples/seurat_object_integrated_OO18_OO20_OO33_02_samples.rds")
...
# ROC test
cluster0.markers <- FindMarkers(seurat_object, ident.1 = 4, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
head(cluster0.markers, n = 20)
cluster0.markers

# visualizing marker expression
VlnPlot(seurat_object, features = c("CD40LG", "IL7R", "CD14", "LEF1", "CD27", "NKG7","GZMA", "FCAR", "MS4A1", "RTKN2", "TMEM40"))

# plot raw counts as well
VlnPlot(seurat_object, features = c("CD40LG", "IL7R", "CD14", "LEF1", "CD27", "NKG7","GZMA", "FCAR", "MS4A1", "RTKN2", "TMEM40"), layer = "data", log = TRUE)

## visualizes feature expression on a umap 
FeaturePlot(seurat_object, features = c("CD40LG", "IL7R", "CD14", "LEF1", "CD27", "NKG7","GZMA", "FCAR", "MS4A1", "RTKN2", "TMEM40"))


# expression heatmap for given cells and features plotting the top 10 markers for each cluster
seurat_object.markers %>%
  group_by(cluster) %>%
...
FeaturePlot(seurat_object, features = c("LTB", "IL7R", "TMEM176A", "BACH2", "TSHZ2", "CCL5", "BNC2", "FCAR","VPREB3","AC093865.1","TMEM40"))


## SingleR annotation
# Get normalized data for SingleR; ensure it has gene names as rownames
# Access normalized data from the integrated assay
normalized_data <- GetAssayData(seurat_object, assay = "integrated", slot = "data")

# Check and set row names to gene names if not already set
if (!identical(rownames(normalized_data), rownames(seurat_object[["integrated"]]))){
  rownames(normalized_data) <- rownames(seurat_object[["integrated"]])
}

# Create a SingleCellExperiment object
sce <- SingleCellExperiment(assays = list(logcounts = as.matrix(normalized_data)))

# Set gene names as row names in the SingleCellExperiment object
rownames(sce) <- rownames(normalized_data)
...
DimPlot(seurat_object, label = TRUE, repel = TRUE, label.size = 5) + NoLegend()
DimPlot(seurat_object, label = TRUE, repel = TRUE, label.size = 4.5)
...
new.cluster.ids <- c("CD4 T", "Naive CD4 T", "CD14+ Mono", "T", "T", "NK_1 subset",
                     "NK_1 subset", "Neutrophils, Macrophages", "B", "CD4 T", "Platelet")
...
ggsave(filename = "C:/Users/Administrator/Desktop/seurat analysis/integrated_OO18_OO20_OO33_samples/integration_manual_annotation_legend_label.jpg", height = 7, width = 12, plot = plot, quality = 50)


# DotPlot for genes expression in each cluster
genes <- c("CD40LG", "CD27", "IL7R", "ARHGAP15", "INPP4B", "CAMK4", "CD14", "FCN1", "LYZ", "LEF1", "NKG7", "GNLY", "CST7", "GZMA", "GZMB", "FCAR",
           "FPR1", "FOLR3", "MS4A1", "FOXP3", "RTKN2","TMEM40")
...
# Save the Seurat object
saveRDS(seurat_object, file = "C:/Users/Administrator/Desktop/seurat analysis/integrated_OO18_OO20_OO33_samples/seurat_object_integrated_OO18_OO20_OO33_03_samples.rds")

# Load the saved Seurat object
seurat_object <- readRDS(file = "C:/Users/Administrator/Desktop/seurat analysis/integrated_OO18_OO20_OO33_samples/seurat_object_integrated_OO18_OO20_OO33_03_samples.rds")
